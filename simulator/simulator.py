...
class GaussianSimulator(object):
    def __init__(self, params, r, phi, batch_size=1, device='cpu'):
        ...
        # Generate pMaps
        self.pMap, self.valid_electrodes = self.generate_pMaps(r, phi) 

        # delete invalid electrodes
        self.pMap = self.pMap[self.valid_electrodes]
        r = r[self.valid_electrodes]
        phi = phi[self.valid_electrodes]
        ...
        if self.thresholding['use_threshold']:
            threshold_mean = self.thresholding['rheobase']*self.thresholding['chronaxie'] #theoretical minimum threshold, intercept of the charge per second version of the strength-duration curve
        ...
        # Set activation mask for sampling from images
    ...
    def generate_pMaps(self, r, phi):
        ...
        print(f"{torch.sum(invalid_electrodes)} phosphenes are outside of view, will be removed. {N_PHOSPHENES-torch.sum(invalid_electrodes)} phosphenes left")
        valid_electrodes = ~invalid_electrodes
        
        # Get pixel-wise distance to phosphene centres
        x = grid[0]-x_offset.view(-1,1,1)
        y = grid[1]-y_offset.view(-1,1,1)

        if self.gabor['gabor_filtering']:
            theta = GABOR_ORIENTATION(N_PHOSPHENES).view(-1,1,1)
        ...
        return pMap, valid_electrodes
    
    def update(self, stim_amp, pw=None, freq=None): 
        """Adjust state as function of previous state and current stimulation""" 

        # If stim is only stimulation amplitude, add other parameters from defaults:
        if pw is None:
            pw = self.default_stim['pw_default']*torch.ones_like(stim_amp)
        if freq is None:
            freq = self.default_stim['freq_default']*torch.ones_like(stim_amp)

        stim = torch.stack((stim_amp,pw,freq),dim=2)
        
        # Calculate input charge per second
        charge_per_second = torch.prod(stim, dim=2) #in Coulomb
        # Calculate effective charge (higher pw or f need higher charge per second to reach threshold)
        ...
        # Update activation using charge per frame and trace for accomodation
        if self.run['video']:
        ...
        if self.size['size_equation'] == 'sqrt':
            AC = (stim[:,:,0]/self.size['current_spread'])**0.5 # Tehovnik et al., 2007
        elif self.size['size_equation'] == 'sigmoid':
            AC = self.size['MD'] / torch.add(torch.exp(-self.size['slope_size']*(stim[:,:,0]-self.size['I_half'])),1) /2 # Bosking et al., 2017
        else:
            raise ValueError("Invalid size equation, should be 'sqrt' or 'sigmoid'")

        # Current spread to sigma in pixels
        radius = AC*1/self.magnification.repeat(self.batch_size,1)
        self.sigma = self.deg2pix_coeff*(radius*self.size['radius_to_sigma'])
        ...
        if self.thresholding['use_threshold']: 
            ...
            self.brightness = spikes*self.brightness
    ...
    def gaussianActivation(self):
        ...
        # Normalise Gaussians so that larger phospenes are not less bright
...
if __name__ == '__main__':
    ...
    ap.add_argument("-nphos", "--n_phosphenes", type=int, default=500, help="number of phosphenes in simulator")
